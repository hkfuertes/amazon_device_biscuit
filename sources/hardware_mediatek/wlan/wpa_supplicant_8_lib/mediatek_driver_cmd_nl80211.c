...
#endif
static int biscuit_read_wlan0_mac(char *buf, size_t buf_len)
{
	...
	if (!f)
		return -1;
	if (!fgets(macaddr, sizeof(macaddr), f)) {
		fclose(f);
		return -1;
	}
	fclose(f);
	for (end = macaddr; *end && *end != '\r' && *end != '\n'; end++)
		;
	*end = '\0';
	return snprintf(buf, buf_len, "Macaddr = %s\n", macaddr);
}
				  ...
				  size_t buf_len )
{
	...
	/* ponytail: Biscuit framework can ask MACADDR before supplicant state is fully wired; sysfs is enough for STA bring-up. */
	if (os_strncmp(cmd, "MACADDR", os_strlen("MACADDR")) == 0) {
		ret = biscuit_read_wlan0_mac(buf, buf_len);
		if (ret >= 0)
			return ret;
	}

	if (bss == NULL) {
		wpa_printf(MSG_ERROR, "%s: bss is NULL. Exiting", __func__);
		return -1;
	}
	drv = bss->drv;
	if (drv == NULL) {
	...
	if (os_strncasecmp(cmd, "POWERMODE ", 10) == 0) {
		...
		/* Already handled through sysfs before touching driver state. */
		ret = -1;
