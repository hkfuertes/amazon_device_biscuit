				  ...
				  size_t buf_len )
{
	...
	if (os_strncmp(cmd, "MACADDR", os_strlen("MACADDR")) == 0) {
		...
		if (f && fgets(macaddr, sizeof(macaddr), f)) {
			char *end = macaddr;
			fclose(f);
			while (*end && *end != '\r' && *end != '\n')
				end++;
...
{
	wpa_printf(MSG_DEBUG, "P2P_SET_NOA %d %d %d, ignored", count, start, duration);
	return -1;
}

int wpa_driver_get_p2p_noa(void *priv, u8 *buf, size_t len)
{
	wpa_printf(MSG_DEBUG, "P2P_GET_NOA, ignored");
	return -1;
}

int wpa_driver_set_p2p_ps(void *priv, int legacy_ps, int opp_ps, int ctwindow)
{
	wpa_printf(MSG_DEBUG, "P2P_SET_PS, ignored");
	return -1;
}
...
{
	wpa_printf(MSG_DEBUG, "set_ap_wps_p2p_ie, ignored");
	return -1;
}
